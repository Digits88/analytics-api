...
function query(instance) {
    const siteGroupId = instance._params.sitegroup,
        siteId = instance._params.site,
...
function processPeriodBucket(processed, delta, singleInterval) {
    if (singleInterval) {
        ...
        return {
            from: processed.from,
            to: processed.to,
            moments: [processed.from],
            displays: [extracted.displays],
...
class VodSiteStat {
    constructor(ctx, search) {
        ...
        if (this._params.starts && this._params.ends) {
            let start_int = parseInt(this._params.starts),
                end_int = parseInt(this._params.ends),
                time_diff = end_int - start_int,
                time_interval = 86400000;
            ...
            if (this._params.previous == 'true') {
                let time_diff_mod = time_diff - (time_diff % time_interval) + time_interval;

                this.range = [(start_int - time_diff_mod), start_int, end_int];
            } else {
...
module.exports.show = async ctx => {
    log.info("site_stats.show(): " + ctx.params.id);
    const client = core(undefined, config, log),
        instance = new VodSiteStat(ctx.query, client.query);
