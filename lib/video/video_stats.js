...
function queries(ctx, now) {
    ...
    const parsed = args.parse(params);
...
module.exports.validateShow = parameterValidator(Joi => {
    return Joi.object().keys({
        start: Joi.date().format(['YYYY-MM-DDTHH', 'YYYY-MM-DD']),
        end: Joi.date().format(['YYYY-MM-DDTHH', 'YYYY-MM-DD'])
    })
});
...
module.exports.show = async ctx => {
    ...
    const video = await videoDb.getOne(ovp.getUserId(ctx), q.parsed.vod);
    if (!video) {
