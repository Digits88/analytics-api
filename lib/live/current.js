...
function queryCurrentViews(ctx) {
    ...
    return {
        size: 0,
        query: util.filter([util.createdBetween(timestamp), util.includingSiteGroups(ovp.getSiteGroupId(ctx)), util.includingSites(ovp.getQuerySites(ctx)), util.includeLiveIds(ctx.query.id)]),
        aggs: {
            sitegroups: {
                terms: {
