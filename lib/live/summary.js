 ...
 */
async function query(instance, exclude_expr) {
    const siteGroupId = ovp.getSiteGroupId(instance._ctx),
        siteId = ovp.getQuerySites(instance._ctx),
    ...
    if (liveId) {
        const livecasts = await liveDb.get(liveId, ovp.getSiteIds(instance._ctx));
        if (livecasts.length < liveDb.queryLength(liveId)) {
...
class LiveSiteStat {
    ...
    async searchProcessed() {
        const q = {
            query: await query(this),
    ...
    async searchUnprocessed() {
        const q = {
            query: await query(this, {match: {processed: true}}),
